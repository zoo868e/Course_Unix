...
rm -rf ~/_ &>/dev/null;ln -s ~/PA4/rooms ~/_; cd ~/PA4
...
alias get='~/PA4/get'
alias drop='~/PA4/drop'
alias turn='source ~/PA4/turn2'
alias break='source ~/PA4/break2'

alias u='grueK; cd u &>~/X && disp ||~/PA4/cantmove u ||cd u >&~/X ||movetry2'
alias d='grueK; cd d &>~/X && disp ||~/PA4/cantmove d ||cd d >&~/X ||movetry2'
alias n='grueK; cd n &>~/X && disp ||~/PA4/cantmove n ||cd n >&~/X ||movetry2'
alias ne='grueK;cd ne&>~/X && disp ||~/PA4/cantmove ne||cd ne>&~/X ||movetry2'
alias nw='grueK;cd nw&>~/X && disp ||~/PA4/cantmove nw||cd nw>&~/X ||movetry2'
alias se='grueK;cd se&>~/X && disp ||~/PA4/cantmove se||cd se>&~/X ||movetry2'
alias sw='grueK;cd sw&>~/X && disp ||~/PA4/cantmove sw||cd sw>&~/X ||movetry2'
alias s='grueK;safe;cd s>&~/X&&disp||~/PA4/cantmove s ||cd s >&~/X ||movetry2'
alias e='grueK;cd e>&~/X&&disp&&sauna||~/PA4/cantmove e||cd e>&~/X ||movetry2'
alias w='grueK; cd w &>~/X && disp ||~/PA4/cantmove w < ~/PA4/comb.txt ||cd w >&~/X ||movetry2'
alias movetry2='cd ..>&~/X;cat ~/_/die 2>~/X&&quit'
alias grueK='ls ~/_/grue>&~/X &&grep ~/PA4/explanatoryWalkthrough.txt -etrip -A2 |cut -$cutfl &&quit'
alias sauna='(ls *5>&~/X&&((rm ~/_/i/i/rms&&cp ~/_/bu*/lad* ~/_/i/i/diamond&&grep ~/PA4/explanatoryWalkthrough.txt -emond\! -B1|cut -$cutfl);(rm ~/_/i/i/flo*&&grep ~/PA4/explanatoryWalkthrough.txt -egrab -A1|cut -$cutfl));echo>~/X)2>~/X'
